...
# Default policy:
# - Only relocate 3 app groups: terminal(kitty/Alacritty/Ghostty), ChatGPT, WeChat
# - Unify Space 3 window features (float/sub-layer/sticky policy)
#
# Mapping:
# - space1: terminal(kitty/Alacritty/Ghostty) (bsp)
# - space2: ChatGPT (bsp)
...
APP_TERM_RE='^(kitty|Alacritty|Ghostty)$'
...
main() {
  ...
  echo "yabai rerange done: only moved terminal(kitty/Alacritty/Ghostty)/ChatGPT/WeChat, unified space3 features"
}
