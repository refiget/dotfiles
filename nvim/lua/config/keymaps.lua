...
-- DAP UI controls
local function dapui_close_by_filetypes(filetypes)
  local wanted = {}
...
local function dapui_has_filetype(ft)
  for _, win in ipairs(vim.api.nvim_list_wins()) do
    local buf = vim.api.nvim_win_get_buf(win)
...
local function dapui_toggle_repl_only()
  ...
  if dapui_has_filetype("dap-repl") then
    dapui_close_by_filetypes({ "dap-repl" })
    return
  end

  dapui.open()
  dapui_close_by_filetypes({ "dapui_scopes", "dapui_breakpoints" })
end

local function dapui_toggle_test_three()
  local ok, dapui = pcall(require, "dapui")
  if not ok then
    vim.notify("dapui not available", vim.log.levels.WARN)
    return
  ...
  local any_open = dapui_has_filetype("dap-repl")
    or dapui_has_filetype("dapui_scopes")
    or dapui_has_filetype("dapui_breakpoints")

  if any_open then
    dapui_close_by_filetypes({ "dap-repl", "dapui_scopes", "dapui_breakpoints" })
  else
    dapui.open()
  end
end

map("n", "<leader>du", dapui_toggle_repl_only, { desc = "DAP UI: toggle REPL only" })
map("n", "<leader>dU", dapui_toggle_test_three, { desc = "DAP UI: toggle scopes+breakpoints+repl" })

-- Java test: keep only REPL visible while running (same effect as <leader>du)
map("n", "<leader>tt", function()
  dapui_toggle_repl_only()
...
end, { desc = "Java: test nearest method (REPL UI mode)" })
